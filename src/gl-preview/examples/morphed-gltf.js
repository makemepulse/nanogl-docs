...
const preview = (canvasEl) => {
  ...
  camera.lens.far = 50;
  camera.position.set([-6, 4, 10]); // set camera back on z axis
  ...
  // load gltf from url
  GltfIO.loadGltf("/src/assets/webgl/models/AnimatedMorphCube.glb").then(async (loadedGltf) => {
    gltf = loadedGltf;
  ...
  const animateGltf = (time = 0) => {
    for (const anim of gltf.animations) {
      // update every animation
      anim.evaluate(time % anim.duration);
    }
    // update gltf world matrix to apply translation/rotation/scale/matrix changes
    gltf.root.updateWorldMatrix();
  }
