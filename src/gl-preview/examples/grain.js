...
const preview = (canvasEl) => {
  ...
  pane.addBinding(PARAMS, 'grainAmount', {
    min: 0,
    max: 1
  }).on('change', () => {
    grain.amount = PARAMS.grainAmount;
  });
  pane.addBinding(PARAMS, 'grainSharpness', {
    min: 0,
    max: 1
  }).on('change', () => {
    grain.sharpness = PARAMS.grainSharpness;
  });
